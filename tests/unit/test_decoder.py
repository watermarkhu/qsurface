...
def test_pymatch():
    ...
    result = decoder.blossom5.getMatching(4, testgraph)
...
def test2d(code, decoder, size, pX):
    decode = getattr(decoder, code)()
    graph = getattr(graph_2D, code)(size, decode)
...
def test_toric3d(code, decoder, size, pX):
    decode = getattr(decoder, code)()
    graph = getattr(graph_3D, code)(size, decode)
    graph.decoder, decode.graph = decode, graph
