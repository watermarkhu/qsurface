...
def union_clusters(parent, child):
    ...
    parent.boundary[1].extend(child.boundary[1])
        ...
        if wind in vertex.neighbors:
            ...
            if new_edge.erasure:
                ...
                if new_vertex.cluster is not cluster:                   # Make sure new bound does not lead to self
                    cluster.boundary[1].append((vertex, new_edge, new_vertex))
...
def grow_clusters(graph, uf_plot=None, plot_step=0):
    ...
    for bucket_i, bucket in enumerate(graph.buckets):
        ...
        for cluster in bucket:                          # Loop over all clusters in the current bucket
            ...
            if cluster.bucket == bucket_i and cluster.boundary[1] != []:  # Check that cluster is not already in a higher bucket and whether this bucket has new boundaries. If not, this cluster is already grown

                cluster.boundary[0] = cluster.boundary[1]    # Set boudary
                ...
                for vertex, new_edge, new_vertex in cluster.boundary[0]:   # Grow boundaries by half-edge
