...
def grow(graph, cluster, root_cluster, support, uf_plot=None, plot_step=0, random_traverse=0, vcomb=0):
    ...
    while cluster.childs[0] != []: # First go through child clusters
        ...
        grow(graph, child_cluster, root_cluster, support, uf_plot, plot_step, random_traverse, vcomb)
    ...
    while cluster.boundary[support] != []:
        ...
        if grrt_cluster is None:  # if new_vertex has no cluster: add to cluster
            ...
            if plot: uf_plot.add_edge(edge, base_vertex)
    ...
    if plot and plot_step: uf_plot.draw_plot(string)
    ...
    for bucket_i, bucket in enumerate(graph.buckets):
        ...
        while bucket != []:                          # Loop over all clusters in the current bucket\
            ...
            if root_cluster.bucket == bucket_i:     # Check that cluster is not already in a higher bucket
                grow(graph, root_cluster, root_cluster, root_cluster.support, uf_plot, plot_step, random_traverse, vcomb)
        if plot and not plot_step: uf_plot.draw_plot("Growing bucket #" + str(bucket_i) + "/" + str(graph.maxbucket) + ".")
    if plot: uf_plot.waitforkeypress("Clusters grown.")



def grow_full(graph, cluster, root_cluster, support, uf_plot=None, plot_step=0, print_steps=0, intervention=0, random_traverse=0, vcomb=0, family_growth=1):
    ...
    if cluster.childs[1] != [] and print_steps: print(cluster, "has fosters:", cluster.childs[1])

    while cluster.childs[1] != []:
        foster_cluster = cluster.childs[1].pop()
        grow_full(graph, foster_cluster, root_cluster, support, uf_plot, plot_step, print_steps, 1, random_traverse, vcomb, 0)

    if family_growth:
        if cluster.childs[0] != [] and print_steps: print(cluster, "has children:", cluster.childs[0])
        while cluster.childs[0] != []: # First go through child clusters
            child_cluster = cluster.childs[0].pop()
            grow_full(graph, child_cluster, root_cluster, support, uf_plot, plot_step, print_steps, intervention, random_traverse, vcomb, 1)
        ...
        if grrt_cluster is None: # if new_vertex has no cluster: add to cluster
            ...
            if not support and edge.support == 2 or support:
                ...
                if intervention and family_growth and grrt_cluster.parity % 2 == 0:
                    ...
                    if print_steps: print("intervention on merge.")
                    break
            elif not support:
                root_cluster.boundary[1].append((base_vertex, edge, grow_vertex))
                if plot: uf_plot.add_edge(edge, base_vertex)
    ...
    if family_growth: cluster.support = 1 - support
    ...
    if print_steps and root_level:
        if not plot: print(string)
        print_cluster = root_cluster if merge_cluster is None else merge_cluster
        graph.print_graph_stop([print_cluster], prestring="A: ")
        if plot and plot_step: uf_plot.waitforkeypress()


def grow_clusters_full(graph, uf_plot=None, plot_step=0, print_steps=0, vcomb=0, random_traverse=0, intervention=0):
    ...
    for bucket_i, bucket in enumerate(graph.buckets):
        ...
        while bucket != []:                          # Loop over all clusters in the current bucket
            ...
            if root_cluster.bucket == bucket_i:                  # Check that cluster is not already in a higher bucket
                if print_steps: graph.print_graph_stop([root_cluster], prestring="B: ")
                grow_full(graph, root_cluster, root_cluster, root_cluster.support, uf_plot, plot_step, print_steps, intervention, random_traverse, vcomb, 1)
            else:
                if print_steps:
        ...
        if print_steps: graph.print_graph_stop(printmerged=0)

        if plot and not plot_step:
            txt = "" if print_steps else "Growing bucket #" + str(bucket_i) + "/" + str(graph.maxbucket) + "."
            uf_plot.draw_plot(txt)
    if plot:
        if print_steps: graph.print_graph_stop()
        if plot_step: uf_plot.waitforkeypress("Clusters grown.")
