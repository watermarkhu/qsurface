...
def cluster_place_bucket(graph, cluster):
    ...
    #         cluster.bucket = place_bucket(cluster.size, graph.bucket_method)
    #
    #     if cluster.support:                      # Additional level added if currently in growth state 1
    #         cluster.bucket += 1
    #
    ...
    #
    #     cluster.bucket = cluster.size - 1 + cluster.support1
    #
...
def find_clusters(graph, uf_plot=None, plot_step=0, anyon_order="random"):
    '''
...
def grow_clusters(graph, uf_plot=None, plot_step=0, print_steps=1, intervention=0):
    ...
    '''
    def grow(graph, cluster, root_cluster, support, uf_plot, plot_step, print_steps, intervention, family_growth=1):
        '''
        ...
        print(cluster, "has fosters:", cluster.childs[1]) if cluster.childs[1] != [] and print_steps else None
        while cluster.childs[1] != []:
            foster_cluster = cluster.childs[1].pop()
            grow(graph, foster_cluster, root_cluster, support, uf_plot, plot_step, print_steps, intervention, False)

        if family_growth:
            print(cluster, "has children:", cluster.childs[0]) if cluster.childs[0] != [] and print_steps else None
            while cluster.childs[0] != []:                                 # First go through child clusters
                child_cluster = cluster.childs[0].pop()
                grow(graph, child_cluster, root_cluster, support, uf_plot, plot_step, print_steps, intervention)
        ...
        cluster.support = 1 - support if family_growth else cluster.support
    ...
    for bucket_i, bucket in enumerate(graph.buckets):
        ...
        while bucket != []:                          # Loop over all clusters in the current bucket
            ...
            if root_cluster.bucket == bucket_i:                  # Check that cluster is not already in a higher bucket
                ...
                grow(graph, root_cluster, root_cluster, root_cluster.support, uf_plot, plot_step, print_steps, intervention)
