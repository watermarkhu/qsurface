...
class lattice_plot:
    ...
    def plot_errors(self, array):
        ...
        for id in [id for id, state in enumerate(array[:self.size*self.size*2]) if not state]:
            Xer.append(id)
        for id in [id for id, state in enumerate(array[self.size*self.size*2:]) if not state]:
            Zer.append(id)
        Yer = []
