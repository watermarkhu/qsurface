...
def cli(args):
    ...
    thres_arguments = [
        ...
        ["-n", "--iterations", "store", "number of iterations per configuration", dict(type=int, default=1)],
        ["-mp", "--mp_processes", "store", "number of multiprocessing threads", dict(type=int, default=1)],
        ["-fc", "--fit_column", "store", "fit threshold of column", dict(type=str)],
        ["-pc", "--plot_column", "store", "plot threshold of column", dict(type=str)],
        ["-o", "--output", "store", "output file name, (none) for no output", dict(type=str, default="")],
        ["-i", "--input", "store", "input file name, use with -fc or -pc to fit and flot existing data", dict(type=str, default="")],
    ]
    thres_error_arguments = [
        ["-px", "--p_bitflip", "store", "Bitflip rate - float {0,1}", dict(type=float,  nargs="*")],
        ["-pz", "--p_phaseflip", "store", "Phaseflip rate - float {0,1}", dict(type=float, nargs="*")],
        ["-pe", "--p_erasure", "store", "Erasure rate - float {0,1}", dict(type=float, nargs="*")],
        ["-pmx", "--pm_bitflip", "store", "Measurement bitflip rate - float {0,1}", dict(type=float, nargs="*")],
        ["-pmz", "--pm_phaseflip", "store", "Measurement phaseflip rate - float {0,1}", dict(type=float, nargs="*")],
    ]
